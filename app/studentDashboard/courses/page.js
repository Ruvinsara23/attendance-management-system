        ...
        {timeTableData.map((item, index) => (
          <div key={index} className="flex items-center gap-4">
            <Avatar className="hidden h-9 w-9 sm:flex">
              ...
              <AvatarFallback>{item.lectureName.split(' ').map(n => n[0]).join('')}</AvatarFallback>
            </Avatar>
            <div className="grid gap-1">
              <p className="text-sm font-medium leading-none">{item.lectureName}</p>
              <p className="text-sm text-muted-foreground">{item.lectureEmail}</p>
            </div>
            <div className="ml-auto font-medium">{item.courseCode}</div>
          </div>
        ))}
      </CardContent>
           </Card>
